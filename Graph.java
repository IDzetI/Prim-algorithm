...
public class Graph<V extends Comparable<V>> {
    ...
    /**
     * create minimal spending tree
	 * using prime's algorithm
     */
    public void mst(){
        PriorityQueue<Road<V>,Double> all = new PriorityQueue<>();

        for (Road road: this.roads) {
            all.add(road,road.getDistance());
        }
        roads.clear();

        while(!all.isEmpty()){
            Road<V> current = all.remove();
            if(searchFastestPath(current.getStart(),current.getEnd())==null){
              roads.add(current);
            }
        }
